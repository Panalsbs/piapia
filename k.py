...
bot = telebot.TeleBot('7537046905:AAEOUslcu5_8eHzCWNuS93HfVViZ8DMR6To')
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = "🚫 Unauthorized Access!🚫Please buy access :-@KRISHNAXLEGEND"
...
def welcome_start(message):
    ...
    response = f'''🫡WELCOME TO KRISHNA DDOS BOT 🚀 '''
    bot.reply_to(message, response)
...
def welcome_rules(message):
    user_name = message.from_user.first_name
    response = f'''
1. ANY PROBLEM DM HERE -> @KRISHNAXLEGEND'''
    bot.reply_to(message, response)
